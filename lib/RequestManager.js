export default class RequestManager {
  ...
  addRequest(requestId, context, cancel) {
    this.log(`CANCELLATION: adding request \`${requestId}\``);
  ...
  removeRequest(requestId, context) {
    this.log(`CANCELLATION: removing request \`${requestId}\``);
  ...
  cancelRequest(
    ...
    if (this.has(requestIdOrContext) && typeof this.pendingRequests[requestIdOrContext].cancel === 'function') {
      this.log(`CANCELLATION: cancelling request \`${requestIdOrContext}\``);
      ...
      this.log(`CANCELLATION: request \`${requestIdOrContext}\` cancelled`);
