export default class CacheManager {
  ...
  isValidResponse(cachedResponse) {
    //cached_until is expected to be a number
  ...
  addResponse(requestId, response) {
    ...
    let cacheTTL = this.options.default_ttl;
    if (response.config.__cache_ttl !== undefined) {
      cacheTTL = response.config.__cache_ttl;
    }
    const responseData = {
      //31536000000 = 1 year
      cached_until: cacheTTL === 0 ? Date.now() + 31536000000 : Date.now() + cacheTTL,
  ...
  isCacheable(config) {
    return config.__cache_ttl !== null && config.__cache_ttl > -1;
  }
}
